...
#if defined(__F16C__) && !(defined(__CUDA_ARCH__) || defined(__CUDACC__) || defined(__HIP_DEVICE_COMPILE__))
#define WITIN_X86_F16 1
 ...
 * between integer and floating-point variables.
 */
INLINE_HOST_DEVICE float fp16_ieee_to_fp32_value(uint16_t h) {
#ifdef WITIN_X86_F16
  return _cvtsh_ss(h);
#else
    /*
...
struct alignas(2) Half {
    ...
    struct from_bits_t {};
    HOST_DEVICE static constexpr from_bits_t from_bits() {
        return from_bits_t();
    }
    ...
    HOST_DEVICE operator float() const;
        ...
        HOST_DEVICE Half(const __half& value);
        HOST_DEVICE operator __half() const;
    #endif
};
